...
public class CsvReportController {
    ...
    @GetMapping("/csv/time")
    @ResponseStatus(HttpStatus.OK)
    public void getByTime(@RequestParam("from") Long from, @RequestParam("to") Long to,
            HttpServletResponse servletResponse) throws IOException {
        servletResponse.setContentType("text/csv");
    ...
    @ResponseStatus(HttpStatus.OK)
    public void getByTopicName(@PathVariable("topicName") String topicName, HttpServletResponse servletResponse)
            throws IOException {
        servletResponse.setContentType("text/csv");
        servletResponse.addHeader("Content-Disposition", "attachment; filename=\"nsi.csv\"");
