...
import miu.edu.swa.pproject.report.dto.MockedData;
...
public class ReportController {
    ...
    @Operation(summary = "Get NSI report by prams filter from mock data")
    @GetMapping("/mock")
    @ResponseStatus(HttpStatus.OK)
    public ArrayList<MockedData> getReportMock(
            @RequestParam(value = "topicName", required = false) String topicName,
            @RequestParam(value = "from", required = false) Long from,
            @RequestParam(value = "to", required = false) Long to) {
        return MockedData.generateMockedData();
    }
